...
class MyKinematics: public BaseSelector {
public:
  explicit MyKinematics(const ParameterSet& config, const TH1* skimCounters);
  virtual ~MyKinematics() {}
...
private:
  ...
  WrappedTH2 *h_DiJet_BJet_dR_Vs_GenTop_Pt;
  //  WrappedTH2 *h_DiJet_BJet_dR_Vs_GenTop_Pt;
  // -------
};

#include "Framework/interface/SelectorFactory.h"
REGISTER_SELECTOR(MyKinematics);

MyKinematics::MyKinematics(const ParameterSet& config, const TH1* skimCounters)
...
void MyKinematics::book(TDirectory *dir) {
  ...
  h_DiJet_BJet_dR_Vs_GenTop_Pt = fHistoWrapper.makeTH<TH2F>(HistoLevel::kVital, th2, "DiJet_BJet_dR_Vs_GenTop_Pt", ";#DeltaR;p_{t}(GeV/c)", nBinsdR, mindR, maxdR, nBinsPt, minPt, maxPt); 
 // --------

  return;
}

void MyKinematics::setupBranches(BranchManager& branchManager) {
  fEvent.setupBranches(branchManager);
}


void MyKinematics::process(Long64_t entry) {
  ...
  for (auto& p: fEvent.genparticles().getGenParticles()) {
	...
	h_DiJet_BJet_dR -> Fill (dR);
	h_DiJet_BJet_dR_Vs_GenTop_Pt -> Fill (dR, genP_pt);
      }
      //std::cout << "---------" << std::endl;
    }   
    // -----------------------------------------------------------------------------
    ...
    // Add table rows
    if (cfg_Verbose) //if(1) to print the table with mothers/daugthers etc     
      {
  ...
  if (cfg_Verbose) table.Print(); //(1) table.Print(); to print the table with mothers/daugthers etc
  ...
  // kchristo --------------------------------------------------------------------
  // --------------------------- Match W's gen jets with W's quarks ---------------

  //For-loop: GenParticles                                                                                                                           
  for (auto& p: fEvent.genparticles().getGenParticles()) {

  // ROOT::Math::XYZPoint genP_vtx(p.vtxX()*10, p.vtxY()*10, p.vtxZ()*10); // in mm                                                                 
    math::XYZTLorentzVector genP_p4, W_fromTop_p4, genJet_from_W_p4 ;
    int genP_pdgId     = p.pdgId();
    genP_p4 = p.p4();

  // Get vectors for daus                                                                                                                       
    std::vector<short> genP_daughters = p.daughters();
  ...
  // find the W
    ...
    if (std::abs(genP_pdgId) == 24) 
      ...
      if (!p.isLastCopy()) continue;
      unsigned int _j = 100 ; //the jet that has already been matched, give an initial big value
      ...
      for (unsigned int i = 0; i < genP_daughters.size() ; i++)
	{
	  ...
	  for (size_t j=0; j < selJets_NoBJets_p4.size(); j++)
	    {
	      if (j == _j) continue; //make sure that you will not match the same jet to both quarks from W
	  ...
	  // for (auto& j: selJets_NoBJets_p4) //prepei na to xannw to selJets_NoBJets_p4 pio panw, error edw
	      // {
	      double deltaR = ROOT::Math::VectorUtil::DeltaR(d.p4(), p4_j); 
	      if (deltaR < _deltaRMin)
		{
		_deltaRMin = deltaR;
		genJet_from_W_p4 = selJets_NoBJets_p4.at(j);
		_j = j;
		}
	    }

	  // add the 2 jets' p4 to create the p4 of the W 
	  //std::cout << genJet_from_W_p4 << std::endl;
	  W_fromTop_p4 = W_fromTop_p4 + genJet_from_W_p4;
	  //std::cout << W_fromTop_p4 << std::endl;
	}
      }
  }// for loop gen particles
...
vector<float> MyKinematics::GetMomentumTensorEigenValues(std::vector<math::XYZTLorentzVector> jets,
						       float &C,
						       float &D,
						       float &H2){
...
vector<float> MyKinematics::GetMomentumTensorEigenValues2D(std::vector<math::XYZTLorentzVector> jets,
							 float &Circularity){
...
vector<float> MyKinematics::GetSphericityTensorEigenValues(std::vector<math::XYZTLorentzVector> jets,
...
TMatrixDSym MyKinematics::ComputeMomentumTensor(std::vector<math::XYZTLorentzVector> jets, double r)
{
...
TMatrixDSym MyKinematics::ComputeMomentumTensor2D(std::vector<math::XYZTLorentzVector> jets)
{
...
double MyKinematics::GetAlphaT(std::vector<math::XYZTLorentzVector> jets,
			     float &HT,
			     float &JT,
			     float &MHT,
			     float &Centrality){
...
vector<GenJet> MyKinematics::GetGenJets(const vector<GenJet> genJets, std::vector<float> ptCuts, std::vector<float> etaCuts, vector<genParticle> genParticlesToMatch)
{
  /*
...
vector<GenJet> MyKinematics::GetGenJets(const GenJetCollection& genJets, std::vector<float> ptCuts, std::vector<float> etaCuts)
{
  std::vector<GenJet> jets;
...
vector<genParticle> MyKinematics::GetGenParticles(const vector<genParticle> genParticles, double ptCut, double etaCut, const int pdgId, const bool isLastCopy, const bool hasNoDaughters)
  {
